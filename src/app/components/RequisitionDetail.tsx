            ...
            <div className="relative">
              ...
              <div className="space-y-4">
                ...
                {req.approvalChain.map((step) => {
                  const isActive = step.status === 'Pending' && step.role === req.currentApproverRole;
                  const isFuture = step.status === 'Pending' && step.role !== req.currentApproverRole;
                  return (
                    <div key={step.id} className={`flex items-start gap-3 relative rounded-lg transition-all ${isActive ? 'bg-amber-50 border border-amber-200 p-2 -mx-2' : ''}`}>
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 z-10 border-2 ${isActive ? 'bg-amber-100 border-amber-400 text-amber-700' : STEP_COLORS[step.status]}`}>
                        {isActive
                      ...
                      <div className="flex-1 pt-1 min-w-0">
                        <div className={`text-sm font-medium ${isActive ? 'text-amber-900' : 'text-slate-800'}`}>
                          {step.status === 'Approved'
                            ? (step.approverName ? `Approved by ${step.approverName}` : `Approved by ${step.label}`)
                            : step.status === 'Rejected'
                            ? `Rejected by ${step.approverName || step.label}`
                            : step.label}
                        </div>
                        {isActive && (
                          <div className="text-amber-700 text-xs font-medium mt-0.5">Action required now</div>
                        )}
                        {isFuture && (
                          <div className="text-slate-400 text-xs">Pending previous approval</div>
                        )}
                      ...
                      }`}>{isActive ? 'Active' : step.status}</span>
                    </div>
                  );
                })}
              </div>
